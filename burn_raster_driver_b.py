...
data_source_distance = '/exports/csce/datastore/geos/users/s1134744/LSDTopoTools/Topographic_projects/shapefiles/simplified_distance_metres_1km.tif'
data_source_distance_along = '/exports/csce/datastore/geos/users/s1134744/LSDTopoTools/Topographic_projects/shapefiles/euc_allocation_5.tif'
data_source_monsoon = '/exports/csce/datastore/geos/users/s1134744/LSDTopoTools/Topographic_projects/TRMM_data/jun_jul_aug.tif'
...
def getRaster(full_directory,summary_directory,fname,write_name,raster_source,raster_name='to_burn',SRTM90=False): 
  ...
  res_90 = "gdalwarp -ot Float64 -of ENVI -tr 90 90 -s_srs 'EPSG:4326' -t_srs"+" '"+ds+"' -te %s %s %s %s %s %s%s_%s.bil" %(LL[0],LL[1],UR[0],UR[1],raster_source,full_directory,fname,raster_name)
  res_30 = "gdalwarp -ot Float64 -of ENVI -tr 30 30 -s_srs 'EPSG:4326' -t_srs"+" '"+ds+"' -te %s %s %s %s %s %s%s_%s.bil" %(LL[0],LL[1],UR[0],UR[1],raster_source,full_directory,fname,raster_name)    
...
for name in name_list:
    ...
    with open(directory+name+'.csv','r') as csvfile:
        ...
        for row in csvReader:
            ...
            #getRaster(full_target,summary_target,fname,write_name,data_source_monsoon,raster_name='monsoon')
            getRaster(full_target,summary_target,fname,write_name,data_source_strain,raster_name='strain_ezz')
            ...
            for x in m_n_list:
                ...
                with open(full_target+fname+"_Chiculations.param",'wb') as file:
                    ...
                    file.write('burn_data_csv_column_header: second_inv \n')
                    file.write("\n")
                    file.write('secondary_burn_raster_to_csv: False \n')
                    file.write('secondary_burn_raster_prefix: %s \n'%(fname+'_distance_along'))
                    file.write('secondary_burn_data_csv_column_header: distance_along \n')
                    file.close()
